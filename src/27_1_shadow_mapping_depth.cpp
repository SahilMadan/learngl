...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glm::mat4 light_view = glm::lookAt(light_position, glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0));
    ...
    // Render depth map to quad for visual debugging. See 19_* examples for
    // more details.
    glViewport(0, 0, kScreenWidth, kScreenHeight);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    screen_shader.Use();
    screen_shader.SetFloat("near_plane", near_plane);
    screen_shader.SetFloat("far_plane", far_plane);
