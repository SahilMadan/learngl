...
int main() {
  ...
  Shader shader("shaders/15_1_depth_testing.vs",
                "shaders/17_1_blending_discard.fs");
  ...
  unsigned int vegetation_vbo;
  glGenBuffers(1, &vegetation_vbo);
  glBindBuffer(GL_ARRAY_BUFFER, vegetation_vbo);
  glBufferData(GL_ARRAY_BUFFER, sizeof(vegetation_vertices),
               vegetation_vertices, GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
  glEnableVertexAttribArray(0);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float),
                        (void*)(3 * sizeof(float)));
  ...
  auto vegetation_texture = LoadTexture("assets/textures/grass.png");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Vegetation
    glBindVertexArray(vegetation_vao);
    glBindTexture(GL_TEXTURE_2D, vegetation_texture);
...
unsigned int LoadTexture(char const* path) {
  ...
  if (data) {
    ...
    // Use GL_CLAMP_TO_EDGE to prevent semi-transparent borders. This happens
    // due to interpolation with the next repeat.
