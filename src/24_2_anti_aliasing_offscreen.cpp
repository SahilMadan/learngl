...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Render: Blit the multisampled buffer to the normal buffer. The reason is
    // because the multisampled buffer contains more information than we need
    // and this must be resolved.
    glBindFramebuffer(GL_READ_FRAMEBUFFER, framebuffer);
    // Specify the draw framebuffer as 0 i.e. the back-buffer.
