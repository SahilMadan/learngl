...
int main() {
  ...
  Shader skybox_shader("shaders/20_2_cubemaps_skybox_optimized.vs",
                       "shaders/20_1_cubemaps_skybox.fs");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Skybox optimization (see vertex shader) means we can render the skybox
    // last, and do not need to render thrown away pixels.
    // To ensure we always draw the skybox furthest away, use depth func less
    // than or equal.
    ...
    // Note: We can remove the translation section of transformation matrices by
    // removing Z column.
...
unsigned int LoadCubemap(std::vector<std::string> faces) {
  ...
  for (unsigned int i = 0; i < faces.size(); i++) {
    unsigned char* data =
        stbi_load(faces[i].c_str(), &width, &height, &channel_count, 0);
    if (data) {
      glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, /*level=*/0, GL_RGB,
                   width, height, /*border=*/0, GL_RGB,
  ...
  // Specify cubemap wrapping and filtering
